...
import type {GreetingsRegistry} from '../typechain-types/index.js';
...
async function main() {
	...
	for (let i = 0; i < messages.length; i++) {
		...
		if (sender) {
			const greetingsRegistryContract = await ethers.getContract<GreetingsRegistry>('GreetingsRegistry', sender);
