...
const {viem, networkConfig, networkName} = await network.connect(undefined, 'optimism');
...
if (networkConfig.type === 'edr') {
	...
	console.log('Note: The forking initialization is not optimized yet, and the example RPC is slower than usual.');
} else {
...
console.log('Sender balance:', await publicClient.getBalance(senderClient.account));
...
console.log(`Transaction included in block ${receipt.blockHash} (#${receipt.blockNumber})`);

if (networkName === 'opSepolia') {
	console.log(`You can check your transaction on https://sepolia-optimism.etherscan.io/tx/${tx}`);
}
