...
describe('Lock', async function () {
	...
	describe('Deployment', function () {
		...
		it('Should fail if the unlockTime is not in the future', async function () {
			...
			).rejects.toThrow(expect.objectContaining({message: 'Unlock time should be in the future'}));
		});
	});
	...
	describe('Withdrawals', function () {
		...
		// 			await assert.rejects(lock.write.withdraw(), (error: Error) => error.message.includes("You can't withdraw yet"));
		// 		});
		...
		// 			await assert.rejects(lockAsOtherAccount.write.withdraw(), (error: Error) =>
		// 				error.message.includes("You aren't the owner"),
		// 			);
		// 		});
		...
		describe('Events', function () {
			it('Should emit an event on withdrawals', async function () {
				...
				assert.equal('amount' in withdrawalEvents[0].args && withdrawalEvents[0].args.amount, lockedAmount);
			});
		});
	});
});
